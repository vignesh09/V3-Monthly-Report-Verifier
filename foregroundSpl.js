...
for (step = 1; step < x.length; step=step+2) {
  ...
  if (feedback_type.includes("Feedback") &&step+1<x.length && x[step+1].getElementsByTagName("td")[6].innerHTML.includes("Feedback")) {
    Feedback.push([feedback_cutoffTime, feedback_type, feedback_content]);
    if (
      feedback_content.split(" ").length < 4 &&
      feedback_content.split(" ").length > 1
    ) {
  ...
  if (feedback_type === "" && step+1<x.length && x[step+1].getElementsByTagName("td")[6].innerHTML === "") {
    No_Feedback.push([feedback_cutoffTime, feedback_type, feedback_content]);
    x[step].setAttribute("style", "background:orange");
    x[step+1].setAttribute("style", "background:orange");
  }
  if (video_viewed_times === " " && step+1<x.length && x[step+1].getElementsByTagName("td")[3].innerHTML === " ") {
